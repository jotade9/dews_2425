...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <!-- detalles de albumess  -->

                <!-- Menú principal panel de albumes  -->
                <?php include 'views/album/partials/menu.albumes.partial.php'; ?>
                                    ...
                                    <td>
                                        ...
                                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                            <a href="<?= URL ?>album/eliminar/<?= $album->id ?>/<?= $_SESSION['csrf_token'] ?>" title="Eliminar"
                                                class="btn btn-danger <?= !in_array($_SESSION['role_id'], $GLOBALS['album']['eliminar']) ? 'disabled' : null ?>"
                                                onclick="return confirm('Confirmar eliminación del álbum')" aria-label="Eliminar álbum">
                                                <i class="bi bi-trash-fill"></i>
                                            </a>
                                            <a href="<?= URL ?>album/editar/<?= $album->id ?>/<?= $_SESSION['csrf_token'] ?>" title="Editar"
                                                class="btn btn-primary <?= !in_array($_SESSION['role_id'], $GLOBALS['album']['editar']) ? 'disabled' : null ?>">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>
                                            <a href="<?= URL ?>album/mostrar/<?= $album->id ?>/<?= $_SESSION['csrf_token'] ?>" title="Mostrar"
                                                class="btn btn-warning <?= !in_array($_SESSION['role_id'], $GLOBALS['album']['mostrar']) ? 'disabled' : null ?>">
                                                <i class="bi bi-eye-fill"></i>
                                            </a>
                                            <!-- Botón que abre el modal -->
                                            <button type="button" class="btn btn-secondary <?= !in_array($_SESSION['role_id'], $GLOBALS['album']['agregar']) ? 'disabled' : null ?>" 
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="modalSubirImagenLabel">Subir Imagen al Álbum</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form action="<?= URL ?>album/agregar/<?= $album->id ?>/<?= $_SESSION['csrf_token'] ?>" method="POST" enctype="multipart/form-data">
                                                            <div class="mb-3">
