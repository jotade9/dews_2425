    ...
    public function validar($param = [])
    {
        ...
        while (($linea = fgetcsv($fichero, 0, ';')) !== FALSE) {
            ...
            if (!$this->model->validateForeignKeyAutor($linea[5])) {
                $_SESSION['mensaje_error'] = 'En la línea' . count($libros) . ' el autor ' . $linea[5] . ' no existe';
                header('location:' . URL . 'libro/importar/csv/' . $_POST['csrf_token']);
                exit();
            }
            // Validar ISBN
            ...
            if (!$this->model->validateForeignKeyEditorial($linea[6])) {
                $_SESSION['mensaje_error'] = 'En la linea ' . count($libros) . ' la editorial ' . $linea[6] . ' no existe';
                header('location:' . URL . 'libro/importar/csv/' . $_POST['csrf_token']);
                exit();
            }
            // validar generos
            ...
            foreach ($generos as $genero) {
                if (!$this->model->validateForeignKeyGenero($genero)) {
                    $_SESSION['mensaje_error'] = 'En la linea ' . count($libros) . ' el género ' . $genero . ' no existe';
                    header('location:' . URL . 'libro/importar/csv/' . $_POST['csrf_token']);
                    exit();
                }
            }
